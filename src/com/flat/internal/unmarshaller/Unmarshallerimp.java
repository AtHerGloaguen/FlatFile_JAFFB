...
public class Unmarshallerimp extends Unmarshaller {
	...
	public Object unmarshal(File file) throws JFFPBException {
		if (!file.exists()) {
			throw new JFFPBException("Fichier n'existe pas ! " + file.getAbsolutePath() + " path ==> "
	...
	private Object fileRunRead(InputStream inputStreamFile, Charset iso) throws JFFPBException {
		...
		Integer itElement = Integer.valueOf(1);
		...
		try (BufferedReader fichierRead = new BufferedReader(new InputStreamReader(inputStreamFile, iso))) {
			...
			if (this.expressioninject == null) {
				...
				for (PositionalMakeRootElem itErableElement : this.positionalMakeRootElems) {
					...
					while (StringUtils.isNotBlank(ligne)) {
						...
						if (!formatRoot.isIslist() && (formatRoot.getStartRowsIterationLigne().equals(itElement))) {
							...
							if (formatRoot.getValuLongueurChaine() != -1
									&& (formatRoot.getValuLongueurChaine() != ligne.length())) {
								throw new JFFPBException("Non conformité de la chaine de caracter Longueur Differente!"
										+ formatRoot.getValuLongueurChaine() + " demandée pour " + ligne.length()
										+ " reel");
							}
							for (FieldPositional field : fdLigneRoot) {
								// passage par chaque élément pour injecter
								// information.
								obj = makeInvokeSetMethodes(obj, field, map.get(field.getNamSetMethode().toLowerCase()),
										ligne);
								// on s'arrette car parcing false
								if (field.getPositionnalJavaTypeAdapter() != null
										&& !field.getPositionnalJavaTypeAdapter().parcing()) {
									break;
								}
							}
							...
							this.object.getClass()
									.getDeclaredMethod(itErableElement.getSetteurMethode(),
											new Class[] { itErableElement.getField().getType() })
							...
							if (ControleInfo.ifConform(formatRoot, ligne)) {
								throw new JFFPBException(
										"Non conformité de la chaine de caractères entete Longueur Differente!"
												+ formatRoot.getValuLongueurChaine() + " demandée pour "
												+ ligne.length() + " reel");
							}
							...
							do {
								...
								for (FieldPositional field : fdLigneRoot) {
									...
									obLigneRoot = makeInvokeSetMethodes(obLigneRoot, field,
											map.get(field.getNamSetMethode().toLowerCase()), ligne);
									// on s'arrette car parcing false
									if (field.getPositionnalJavaTypeAdapter() != null
											&& !field.getPositionnalJavaTypeAdapter().parcing()) {
										break;
				...
				while (StringUtils.isNotBlank(ligneElse)) {
					...
					if (patternExpr.getElement().getFormatRootElem().getValuLongueurChaine() != -1 && (patternExpr
							.getElement().getFormatRootElem().getValuLongueurChaine() != ligneElse.length())) {
						throw new JFFPBException("Non conformité de la chaine de caracter entete Longueur Differente! "
								+ patternExpr.getElement().getFormatRootElem().getValuLongueurChaine()
								+ " demander pour " + ligneElse.length() + " reel");
					}
					...
					if (!patternExpr.getElement().getFormatRootElem().isIslist()) {
						...
						for (FieldPositional field : patternExpr.getFd_LigneRoot()) {
							...
							makeInvokeSetMethodes(obLigneRoot, field,
									patternExpr.getMap().get(field.getNamSetMethode().toLowerCase()), ligneElse);
				...
				for (Object pattern2 : keyExpre) {
					...
					this.object.getClass()
							.getDeclaredMethod(this.expressioninject.get(pattern2).getElement().getSetteurMethode(),
									new Class[] {
											this.expressioninject.get(pattern2).getElement().getField().getType() })
		...
		try {
			if (this.expressioninject == null) {
				...
				while (((ligne = (fichierRead.hasNext() ? fichierRead.next() : null))) != null) {
					...
					// contréle de la longueur
					if (patternExpr.getElement().getFormatRootElem().getValuLongueurChaine() != -1 && (patternExpr
							.getElement().getFormatRootElem().getValuLongueurChaine() != ligne.length())) {
						throw new JFFPBException("Non conformité de la chaine de caracter entete Longueur Differente! "
								+ patternExpr.getElement().getFormatRootElem().getValuLongueurChaine()
								+ " demander pour " + ligne.length() + " reel");
					}
