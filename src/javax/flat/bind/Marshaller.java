...
public abstract class Marshaller extends CommunContext {
	 ...
	 * avec les valeurs de l'objet passé en paramétre
	 */
	public abstract void marshal(Object object, OutputStream out, Charset is, Boolean carriageReturn)
	 ...
	 * avec les valeurs de l'objet passé en paramétre
	 */
	public abstract void marshalCsv(Object object, File fichier) throws JFFPBException;

	/**
	 * génére le fichier passé en paramétre,<br />
	 * avec les valeurs de l'objet passé en paramétre
	 */
	public abstract void marshalCsv(Object object, File fichier, Charset iso) throws JFFPBException;

	/**
	 * génére le OutputStream passé en paramétre,<br />
	 * avec les valeurs de l'objet passé en paramétre
	 */
	public abstract void marshalCsv(Object object, OutputStream out) throws JFFPBException;

	/**
	 * génére le OutputStream passé en paramétre,<br />
	 * avec les valeurs de l'objet passé en paramétre
	 */
	public abstract void marshalCsv(Object object, OutputStream out, Charset is) throws JFFPBException;

	/**
	 * génére le fichier passé en paramétre,<br />
	 * avec les valeurs de l'objet passé en paramétre
	 */
	public abstract void marshalCsv(Object object, File fichier, Boolean carriageReturn) throws JFFPBException;

	/**
	 * génére le fichier passé en paramétre,<br />
	 * avec les valeurs de l'objet passé en paramétre
	 */
	public abstract void marshalCsv(Object object, File fichier, Charset is, Boolean carriageReturn)
			throws JFFPBException;
	 ...
	 * avec les valeurs de l'objet passé en paramétre
	 */
	public abstract void marshalCsv(Object object, OutputStream out, Boolean carriageReturn) throws JFFPBException;

	/**
	 * génére le OutputStream passé en paramétre,<br />
	 * avec les valeurs de l'objet passé en paramétre
	 */
	public abstract void marshalCsv(Object object, OutputStream out, Charset is, Boolean carriageReturn)
			throws JFFPBException;
	 ...
	 * @throws Exception
	 */
	public String convertObjectInStringChaine(Object instanceObject, Integer longueur, Properties desactivat)
			throws JFFPBException {
	 ...
	 * @throws Exception
	 */
	public String convertObjectInStringChaine(Object instanceObject, Character caractereRemplissage, Integer longueur)
			throws JFFPBException {
	 ...
	 */
	public String convertObjectInStringChaine(Object instanceObject, Character caractereRemplissage, Integer longueur,
			Properties desactivat) throws JFFPBException {
	...
	public String convertObjectInChaineCsv(Object data, String caractereSeparateur) throws JFFPBException {
		...
		for (Integer entryfdLigneRoot : keyListField) {
			...
			buffer = (StringBuffer) makeInvokeCsvGetMethode(buffer, data, fDligneRoot.get(entryfdLigneRoot).getField(),
					caractereSeparateur);
	 ...
	 * @throws Exception
	 */
	public String convertObjectInChaineCsv(List<Object> data, String caractereSeparateur, Boolean header) throws JFFPBException {
		...
		if (header) {
			...
			ExtractHeadCsv extractHeadCsv = new ExtractHeadCsv(data.get(0).getClass(), caractereSeparateur );
			builder.append(extractHeadCsv.getHeaderForClass()+"\n") ;
			
		}

		for (Object ojb : data) {
			builder.append(convertObjectInChaineCsv(ojb, caractereSeparateur)+"\n") ;
		}
		
		return builder.toString() ;
	}

	protected String makeInvokeWriteMethodes(StringBuffer buffer, Object ob, Field[] fd, boolean longeurIndiquer,
			Properties desactivat) throws JFFPBException {
	 ...
	 * @throws Exception
	 */
	private Object makeInvokeGetMethodes(StringBuffer buffer, Object ob, Field[] fd, boolean longeurIndiquer,
			Properties desactivat) throws JFFPBException {
		...
		for (Field field : fd) {
			...
			PositionalMappingParse positionnalMappingParce = PositionalMakeAnnotation
					.getFieldPositionnalMappingParse(field);
			...
			int depart = 0;
			if (longeurIndiquer) {
				depart = ((mapForVariant.containsKey(field.getName()) ? mapForVariant.get(field.getName())
						: positionnalMappingParce.offset()) - 1) - ifDesactiRecalc;
			...
			PositionalJavaTypeAdapter positionnalJavaTypeAdapter = PositionalMakeAnnotation
					.getFielPositionnalJavaTypeAdapter(field);
			...
			if (valu != null) {
				...
				PositionalControlRegex positionnalControlRegex = PositionalMakeAnnotation
						.getFielPositionnalControlRegex(field);
				if (positionnalControlRegex != null) {
					...
					if (!Pattern.compile(positionnalControlRegex.expression()).matcher(obInvoke.toString()).find()) {
						if (positionnalMappingParce.required()) {
							...
							throw new JFFPBException("Non conformité de la chaine de caracter ! '" + obInvoke.toString()
									+ "' pour l'expression " + positionnalControlRegex.expression());
						}
				...
				if (positionnalMappingParce.padding() == -1) { // top
					...
					buf = new StringBuffer(
							StringUtils.repeat(positionnalMappingParce.charcatereRepliForPadding(), moduloDep));
					...
					buf = new StringBuilder(
							StringUtils.repeat(positionnalMappingParce.charcatereRepliForPadding(), longueurChaine));
					buf.append(obInvoke.toString());
					int calcCut = buf.length() - longueurChaine;
					buffer.replace(depart, depart + longueurChaine,
							buf.toString().substring(calcCut, longueurChaine + calcCut));
	...
	private Object makeInvokeCsvGetMethode(StringBuffer buffer, Object ob, Field field, String caractereSeparateur)
			throws JFFPBException {
		...
		if (csvMappingParse != null) {
			...
			if (Valu != null) {
				if (!(Valu instanceof List)) {
					...
					if (positionnalJavaTypeAdapter != null) {
						obInvoke = PositionalMakeAnnotation.typeAdapter(Valu, positionnalJavaTypeAdapter.value(),
								PARSING);
					...
					if (positionnalControlRegex != null) {
						// si la ligne est conform
						if (!Pattern.compile(positionnalControlRegex.expression()).matcher(obInvoke.toString())
								.find()) {
							if (csvMappingParse.required()) {

								throw new JFFPBException(
										"Non conformité de la chaine de caracter ! '" + obInvoke.toString()
												+ "' pour l'expression " + positionnalControlRegex.expression());
							}
							getAfficheLog("value Non conforme ==> " + obInvoke.toString() + " pour l'expression ==> "
									+ positionnalControlRegex.expression());
					...
					if (lists.size() > 0 && (buffer.length() > 1 || buffer.length() == 1)
							&& buffer.toString().endsWith(caractereSeparateur)) {
